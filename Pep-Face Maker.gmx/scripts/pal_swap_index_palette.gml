var _spr, _colors, _palettes, _num, _surface, tempi, _pal, ii;
if (! ds_map_exists(global.Pal_Map, argument[0]))
{
    ...
    for (tempi = 0; tempi < _num; tempi = (tempi + 1))
    {
        _pal = ds_list_create()
        for (ii = 0; ii < _colors; ii = (ii + 1))
            ds_list_add(_pal, surface_getpixel(_surface, tempi, ii))
